#include <QSize>
// Qt 6.6+ uses the rhi module under QtGui/rhi
...
RTXUpscaleNode::RTXUpscaleNode(const std::string &nodeId) : BaseNodeBackend(nodeId, "RTXUpscale")
{
	...
	m_scaleFactor = Rendering::RTXUpscaler::ScaleFactor::Scale_2x;
}
...
bool RTXUpscaleNode::initialize(const NodeConfig &config)
{
	...
	switch (scale) {
	case 1:
		m_scaleFactor = Rendering::RTXUpscaler::ScaleFactor::Scale_1_33x;
		break;
	case 2:
		m_scaleFactor = Rendering::RTXUpscaler::ScaleFactor::Scale_2x;
		break;
	case 3:
		m_scaleFactor = Rendering::RTXUpscaler::ScaleFactor::Scale_3x;
		break;
	case 4:
		m_scaleFactor = Rendering::RTXUpscaler::ScaleFactor::Scale_4x;
		break;
	default:
		m_scaleFactor = Rendering::RTXUpscaler::ScaleFactor::Scale_2x;
	}
...
ExecutionResult RTXUpscaleNode::process(ExecutionContext &ctx)
{
	...
	if (!m_upscaler && ctx.renderer) {
		...
		if (!m_upscaler->initialize(inputSize.width(), inputSize.height(), m_scaleFactor, m_qualityMode)) {
...
REGISTER_NODE_TYPE(RTXUpscaleNode, "RTXUpscale")
