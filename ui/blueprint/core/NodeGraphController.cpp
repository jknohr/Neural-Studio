...
#include <optional>
// Updated include paths for new project structure
// From ui/blueprint/core -> ../../../ = project root
...
{
	// Initialize the backend graph directly
	m_backendGraph = std::make_shared<SceneGraph::NodeExecutionGraph>();
}

NodeGraphController::~NodeGraphController()
{
	// NodeExecutionGraph doesn't require explicit cleanup
}
...
QString NodeGraphController::createNode(const QString &nodeType, float x, float y, float z)
{
	...
	std::string id = "node_" + std::to_string(m_backendGraph->getNodeCount() + 1);
	while (m_backendGraph->getNode(id)) {
		id = "node_" + std::to_string(std::rand());
	}

	// Create node via factory
	auto node = SceneGraph::NodeFactory::create(nodeType.toStdString(), id);
	if (node) {
...
{
	if (m_backendGraph) {
		m_backendGraph->removeNode(nodeId.toStdString());
...
{
	bool success = m_backendGraph &&
		       m_backendGraph->connectPins(sourceNodeId.toStdString(), sourcePinId.toStdString(),
...
{
	if (m_backendGraph) {
		m_backendGraph->disconnectPins(targetNodeId.toStdString(), targetPinId.toStdString());
...
void NodeGraphController::compileAndRun()
{
	...
	bool success = m_backendGraph->compile();
	emit isCompiledChanged();

	if (success) {
		// Execute one frame with a context
		SceneGraph::ExecutionContext ctx;
		ctx.deltaTime = 0.016f; // 60fps
		ctx.frameNumber = 0;
		m_backendGraph->execute(ctx);
...
void NodeGraphController::setNodeProperty(const QString &nodeId, const QString &propertyName, const QVariant &value)
{
	...
	auto node = m_backendGraph->getNode(nodeId.toStdString());
	if (!node) {
